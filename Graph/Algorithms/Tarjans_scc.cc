					...
					stack<int>* st, bool stackMember[])
{
	...
	st->push(u);
	stackMember[u] = true;
	//cout << "Start Processing  " << u << " with disc and low "<< time << endl;
	//cout << u << " is inserted into stack." << endl;
	...
	for (i = adj[u].begin(); i != adj[u].end(); ++i) {
		...
		if (disc[v] == -1) {
            ...
            //cout << "For " << u << " New Unvisited Neighbor " << v << endl;
		...
		{
		    //cout << "For " << u << " visited neighbor " << v << endl;
			low[u] = min(low[u], disc[v]);
		}
	    //cout << "Low[u] after processing neighbor v is " << low[u] << endl;
	}

	// head node found, pop the stack and print an SCC
	int w = 0; // To store stack extracted vertices
	//cout << "After processing " << u << " low " << low[u] << " disc " << disc[u] << endl;
	if (low[u] == disc[u]) {
		while (st->top() != u) {
			w = (int)st->top();
			cout << w << " ";
